...
/**
 * Cross-platform flag icons.
 * - Web: SVG data URIs (emoji flags break on Windows/Replit iframe)
 * - Native (Expo Go): Emoji flags (RN Image doesn't support SVG data URIs)
 */

// Simplified flag SVGs as data URIs - works on web only
...
export default function FlagIcon({ languageCode, size = 24 }: FlagIconProps) {
  // On web: use SVG data URIs (emoji flags break on many web environments)
  if (Platform.OS === 'web') {
  ...
  // On native (Expo Go): always use emoji flags (RN Image doesn't support SVG data URIs)
