...
function bindEvents() {
  ...
  $("input-admin-pin").addEventListener("keyup", (e) => {
...
function renderParentProfiles() {
  ...
  list.innerHTML = S.users
    .map((u) => {
      ...
      const countLabel =
        childCount === 1
          ? t("child_count", { n: childCount })
          : t("children_count", { n: childCount });
      return `
...
function computeHomeStats(children) {
  let totalDone = 0,
    totalPending = 0;
  const alerts = [],
    upcoming = [];
  ...
  return { totalDone, totalPending, alerts, upcoming };
}
...
function renderHomeChildren(children) {
  ...
  if (!isEmpty) {
    ...
    $("home-children").innerHTML = children
      .map((child, i) => {
        ...
        const overdueCount = vaxes.filter(
          (v) =>
            !v.completedDate && vaccineStatus(v, child.birthDate) === "overdue",
        ).length;
        const pendingCount = vaxes.filter(
          (v) =>
            !v.completedDate && vaccineStatus(v, child.birthDate) === "pending",
        ).length;
        const upcomingCount = vaxes.filter(
          (v) =>
            !v.completedDate &&
            vaccineStatus(v, child.birthDate) === "upcoming",
        ).length;
...
function renderHomeUpcoming(alerts, upcoming) {
...
function renderHome() {
  ...
  const { totalDone, totalPending, alerts, upcoming } =
    computeHomeStats(children);
  ...
  renderHomeChildren(children);
  renderHomeUpcoming(alerts, upcoming);
}
